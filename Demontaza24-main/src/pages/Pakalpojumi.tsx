...
export default function Pakalpojumi() {
  ...
  return (
    <main className="w-full bg-white text-zinc-900">
      ...
      <section className="py-20" aria-label="Pakalpojumu detaļas">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 space-y-24">
          
          {SERVICES_DATA.map((service) => (
            <article 
              key={service.id} 
              className="bg-white border border-zinc-200 p-6 sm:p-10 flex flex-col lg:flex-row gap-8 items-start transition-all hover:border-[#FBBF24] hover:shadow-lg rounded-[2px] group"
              aria-label={service.title}
            >
              
              {/* Left Column: Image with dynamic lazy load */}
              <div className="w-full lg:w-2/5 overflow-hidden border border-zinc-150 h-64 sm:h-72 flex-shrink-0 rounded-[2px]">
                <img
              ...
              <div className="w-full lg:w-3/5 space-y-5 flex flex-col justify-between h-auto">
                <div className="space-y-4">
                  <div className="flex items-center gap-3.5">
                    <div className="relative p-3 bg-zinc-950 text-[#FBBF24] border border-[#FBBF24]/30 rounded-[2px] shadow-md transition-all duration-300 group-hover:border-[#FBBF24] flex-shrink-0">
                    ...
                    <h2 className="text-zinc-950 text-[18px] sm:text-xl font-bold tracking-tight uppercase group-hover:text-[#FBBF24] transition-colors">
                      {service.title}
                    </h2>
                  </div>
                  ...
                  {/* Bullets lists */}
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-zinc-500 font-sans text-xs pt-1 border-t border-zinc-100 pt-3">
                ...
                <div className="pt-4 mt-auto">
                  <button
                    ...
                    className="cursor-pointer inline-flex items-center justify-center bg-zinc-100 hover:bg-[#FBBF24] text-zinc-900 hover:text-zinc-950 border border-zinc-250 px-7 py-3.5 text-xs font-black uppercase tracking-wider transition-all duration-150 rounded-[2px] shadow-sm gap-2 w-full sm:w-auto"
                    aria-label={`Pieteikt tāmēšanu - ${service.title}`}
                  >
                    <span>Pieteikt tāmēšanu</span>
