...
async function run() {
  const res = await fetch("https://pub-ff8b54c4ee504990b655b0d624a4449e.r2.dev/Logo-demontaza24.webp");
  const buf = await res.arrayBuffer();
  
  const img = sharp(Buffer.from(buf));
  const { data, info } = await img.ensureAlpha().raw().toBuffer({ resolveWithObject: true });

  // Let's analyze the bottom 25% of the image height where "demontaza" and "eu" normally reside
  // Height = 1024, so lets examine y from 700 to 1024
  ...
  console.log(`Bottom portion stats:`);
